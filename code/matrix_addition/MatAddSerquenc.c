...
int main(){
	...
	double time_taken;			// variable to mesure the time algorith took to run 
	int i, j, N;				// other variables 
...
*/
	...
	for(i=0;i<N;i++){					
		for (j=0;j<N;j++)
		C[i][j] = A[i][j] + B[i][j];				// Algorithm which make the Addition Sequencialy
	}
	end = clock();									// Stopping the clock

	// display2D(C, N);								// Function to display Matrix C
	
	time_taken = (end-start);						// clock is counting by subtracting end from start
	time_taken /= CLOCKS_PER_SEC;					// time_taken will be devided by CLOCKS_PER_SEC function to give us time used in seconds
	printf("\nExecution τime of sum:"YEL" %5.6f,"RESET" N was "RED"%d\n\n"RESET, time_taken, N);
